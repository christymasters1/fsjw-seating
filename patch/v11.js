...
(function(){
  ...
  function filterReservations(sidebar){
    const q=searchQuery.trim().toLowerCase();
    sidebar.querySelectorAll('.resitem').forEach(function(card){
  ...
  document.addEventListener('DOMContentLoaded',ensureStaticSearch);
  const previousRenderAll=window.renderAll;
  if(typeof previousRenderAll==='function'){
    window.renderAll=function(){previousRenderAll();ensureStaticSearch();};
  }
  setTimeout(ensureStaticSearch,0);
})();