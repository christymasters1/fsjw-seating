...
(function(){
  ...
  async function loadHistory(){
    ...
    try{
      ...
      historyReady=true;
    }catch(e){console.error("Historical seating load failed",e);}
  ...
  function historicalDetailsMarkup(rezId){
    ...
    return '<section class="history2025Native"><div class="history2025NativeTitle">2025 HISTORY</div><div class="detailLabel">2025 Seating</div><div class="detailText">'+seats+'</div><div class="detailLabel">2025 Comments</div><div class="detailText">'+comments+'</div><div class="detailLabel">2025 Linked Guests</div><div class="detailText">'+linked+'</div></section>';
  }
  window.historical2025DetailsMarkup=historicalDetailsMarkup;

  function reservationSidebar(){return document.querySelector(".sidebar");}
  function findNotesHeading(){
    const sidebar=reservationSidebar();if(!sidebar)return null;
    const candidates=[...sidebar.querySelectorAll("h1,h2,h3,h4,h5,h6,div,span,strong,b")];
    return candidates.find(el=>/^christy\s*\+\s*cathy\s+notes$/i.test(String(el.textContent||"").trim()))||null;
  }
  function ensureHistoryMount(){
    const sidebar=reservationSidebar();if(!sidebar)return;
    const notesHeading=findNotesHeading();if(!notesHeading)return;
    let mount=sidebar.querySelector("#history2025Mount");
    if(!mount){mount=document.createElement("div");mount.id="history2025Mount";notesHeading.parentNode.insertBefore(mount,notesHeading);}
    else if(mount.nextSibling!==notesHeading){notesHeading.parentNode.insertBefore(mount,notesHeading);}
    const rez=currentRez();
    const version=rez+"|"+(historyReady?"1":"0")+"|"+historicalRecords.length+"|"+historicalLinks.length;
    if(mount.dataset.version!==version){mount.innerHTML=historicalDetailsMarkup(rez);mount.dataset.version=version;}
  }
  function scheduleHistoryRender(){clearTimeout(renderTimer);renderTimer=setTimeout(()=>{ensureHistoryMount();ensureToggle();overlaySeatMap();},0);}
  ...
  function ensureToggle(){
    ...
    if(!btn){btn=document.createElement("button");btn.id="toggle2025Seating";btn.type="button";btn.className="toggle2025Seating";btn.addEventListener("click",()=>{overlayOn=!overlayOn;btn.textContent=overlayOn?"Hide 2025 Seating":"Show 2025 Seating";btn.classList.toggle("active",overlayOn);overlaySeatMap();});const legend=map.querySelector(".maplegend");if(legend)legend.insertAdjacentElement("afterend",btn);else map.insertBefore(btn,map.firstChild);}
    btn.textContent=overlayOn?"Hide 2025 Seating":"Show 2025 Seating";btn.classList.toggle("active",overlayOn);
  }

  function buildHistoricalLinks(records){
    const links=[],matched=records.filter(r=>r.matched_rez_id),candidates=matched.map(r=>({rez:String(r.matched_rez_id),full:norm(r.guest_name),last:norm(r.guest_name).split(" ").filter(Boolean).pop()||""})),lastCounts=new Map(),seen=new Set();
    candidates.forEach(c=>{if(c.last.length>=5)lastCounts.set(c.last,(lastCounts.get(c.last)||0)+1);});
    matched.forEach(source=>{const text=norm((source.reservation_comments||"")+" "+(source.guest_comments||""));if(!text||!/(seat|sit|room|near|beside|with|group|friend|together|next to)/i.test(text))return;candidates.forEach(c=>{if(c.rez===String(source.matched_rez_id))return;const fullHit=c.full.length>=6&&text.includes(c.full);const lastHit=!fullHit&&c.last.length>=5&&lastCounts.get(c.last)===1&&new RegExp("\\b"+c.last+"s?\\b","i").test(text);if(!fullHit&&!lastHit)return;const a=String(source.matched_rez_id),b=c.rez,key=[a,b].sort().join("|");if(seen.has(key))return;seen.add(key);links.push({event_year:HISTORY_YEAR,matched_rez_id:a,linked_rez_id:b,source_historical_rez_id:source.historical_rez_id,basis:fullHit?"2025 comment named guest":"2025 comment named unique surname"});});});
    return links;
  }
  ...
  async function importHistory(file){
    ...
    const links=buildHistoricalLinks(records);if(links.length)await chunkedUpsert("historical_links",links,"event_year,matched_rez_id,linked_rez_id",100);
    historicalRecords=records;historicalLinks=links;historyReady=true;scheduleHistoryRender();
    return{records:records.length,matched:records.filter(r=>r.matched_rez_id).length,links:links.length};
  }

  function ensureImportButton(){
    const actions=document.querySelector(".actions");if(!actions)return;
    let old=document.getElementById("import2025Btn");if(old&&old.dataset.nativeHistory==="1")return;
    ...
    let input=document.getElementById("import2025NativeFile");if(!input){input=document.createElement("input");input.id="import2025NativeFile";input.type="file";input.accept=".csv,text/csv";input.style.display="none";actions.appendChild(input);}if(!old)actions.appendChild(btn);
    btn.onclick=()=>input.click();
    input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{setStatus("Reading 2025 historical seating locally…","busy");const r=await importHistory(file);setStatus("Imported "+r.records+" 2025 guest records; "+r.matched+" matched to 2026; "+r.links+" historical links found.","ok");}catch(e){console.error(e);setStatus("2025 import failed: "+e.message,"warn");}finally{input.value="";}};
  }
  ...
  if(typeof priorRender==="function")window.renderAll=function(){priorRender();ensureImportButton();scheduleHistoryRender();};

  const style=document.createElement("style");style.textContent=`
    #history2025Mount{width:100%}
  ...
  function installObserver(){
    const sidebar=reservationSidebar();if(!sidebar)return;
    const observer=new MutationObserver(()=>scheduleHistoryRender());
    observer.observe(sidebar,{childList:true,subtree:true});
  }

  loadHistory().then(()=>{ensureImportButton();scheduleHistoryRender();});
  document.addEventListener("DOMContentLoaded",()=>{ensureImportButton();installObserver();scheduleHistoryRender();});
  if(document.readyState!=="loading"){ensureImportButton();installObserver();scheduleHistoryRender();}
})();